...
def create_map(parsed_coordinates):
    ...
    max_y += 2
    new_max_x = min_x + max_width(max_y) - 1 + 4
    additional_width = new_max_x - max_x
    max_x = new_max_x
    output = np.full((max_y - min_y + 1, max_x - min_x + 1), ".")
    sand_source = (sand_source[0], sand_source[1] - min_x + additional_width // 2 - 1)
    ...
    for line in parsed_coordinates:
        for i, (x, y) in enumerate(line):
            ...
            output[
                ...
                (x_smaller - min_x + additional_width // 2 - 1) : (
                    x_larger - min_x + 1 + additional_width // 2 - 1
                ),
            ] = "#"
    return output
