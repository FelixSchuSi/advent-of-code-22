...
input = open("11/example-input", "r").read()
...
def simulate_round(parsed_monkeys, round_idx):
    print(f"Round {round_idx + 1}:")
    for monkey in parsed_monkeys:
        for item_to_inspect in monkey["items"]:
            monkey["items_inspected"] += 1
            new_item_to_inspect = monkey["operation"](item_to_inspect) // 3
...
for round_idx in range(20):
    parsed_monkeys = simulate_round(parsed_monkeys, round_idx)
